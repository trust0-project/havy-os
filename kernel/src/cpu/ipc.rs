...
use crate::cpu::process::Pid;
use crate::Spinlock;
use crate::services::klogd::{klog_debug, klog_trace};
...
impl IpcRegistry {
    ...
    pub fn create_pipe(&self) -> Arc<Pipe> {
        ...
        klog_debug("ipc", &alloc::format!("Created pipe (id={})", id));
    ...
    pub fn remove_pipe(&self, id: PipeId) {
        if let Some(pipe) = self.pipes.lock().remove(&id) {
            pipe.close_read();
            pipe.close_write();
            klog_debug("ipc", &alloc::format!("Removed pipe (id={})", id));
        }
    }
