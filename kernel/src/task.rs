...
// Re-export WaitQueue types from lock::state::waitq for backwards compatibility
pub use crate::lock::state::waitq::{
    Pid,
    WaitEvent,
    WaitQueueState as WaitQueue,
};
...
pub fn wake_ipc(_channel_id: u64) -> usize {
    // Note: This function previously accessed wq.waiters directly which is not public
    // A proper implementation would need the WaitQueue to expose a wake_by_data method
    if let Some(ref wq) = *IPC_WAITQ.lock() {
        wq.wake_event(WaitEvent::IpcMessage)
    } else {
        0
    }
}
