...
impl NetState {
    ...
    pub fn new(mut device: D1Emac) -> Result<Self, &'static str> {
        ...
        let seed = {
            ...
            let ip_part = (ip_octets[0] as u64) << 24 
                        ...
                        | (ip_octets[3] as u64);
            let time_part = crate::clint::get_time_ms() as u64;
        ...
        let icmp_rx_buffer = unsafe { icmp::PacketBuffer::new(&mut ICMP_RX_META[..], &mut ICMP_RX_DATA[..]) };
        let icmp_tx_buffer = unsafe { icmp::PacketBuffer::new(&mut ICMP_TX_META[..], &mut ICMP_TX_DATA[..]) };
        let mut icmp_socket = icmp::Socket::new(icmp_rx_buffer, icmp_tx_buffer);
        icmp_socket.bind(icmp::Endpoint::Ident(ICMP_IDENT)).ok();

        // Create UDP socket
        let udp_rx_buffer = unsafe { udp::PacketBuffer::new(&mut UDP_RX_META[..], &mut UDP_RX_DATA[..]) };
        let udp_tx_buffer = unsafe { udp::PacketBuffer::new(&mut UDP_TX_META[..], &mut UDP_TX_DATA[..]) };
    ...
    pub fn tcp_listen(&mut self, port: u16) -> Result<TcpSocketId, &'static str> {
        ...
        let (rx_data, tx_data) = unsafe {
            ...
            (&mut TCP_SERVER_RX_DATA[idx][..], &mut TCP_SERVER_TX_DATA[idx][..])
        };
        ...
        -> Result<(), &'static str> 
    {
        ...
        // Get our IP and pick a local port
        let local_ip = get_my_ip();
        let local_port = 49152 + ((timestamp_ms as u16) % 16384); // Ephemeral port
    ...
    /// Alias for tcp_client_state (backward compatibility)
    pub fn tcp_state(&mut self) -> &'static str {
        self.tcp_client_state()
    }
