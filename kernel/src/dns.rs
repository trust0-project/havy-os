...
/// High-level DNS resolution function for D1 EMAC network state
///
/// This performs a DNS lookup using the provided D1NetState.
/// Returns the first resolved IPv4 address or None on failure.
pub fn resolve_d1(
    net: &mut crate::net::D1NetState,
    hostname: &[u8],
    dns_server: Ipv4Address,
    timeout_ms: i64,
    get_time_ms: fn() -> i64,
    ...
    if net
        ...
        .is_err()
    {
        uart::write_line("Failed to send DNS query (D1 EMAC)");
        return None;
    }
    ...
    loop {
        let now = get_time_ms();
        if now - start_time > timeout_ms {
            uart::write_line("DNS query timed out (D1 EMAC)");
            return None;
        }
        ...
        if let Some((_src_ip, _src_port, len)) = net.udp_recv(&mut buf, now) {
            match parse_response(&buf[..len], txid) {
                ...
                DnsResult::NotFound => {
                    uart::write_line("DNS: domain not found (D1 EMAC)");
                    return None;
                }
                DnsResult::Error(e) => {
                    uart::write_str("DNS error (D1 EMAC): ");
                    uart::write_line(e);
                    return None;
                }
                DnsResult::WrongId => {
